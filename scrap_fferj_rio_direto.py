...
FFERJ_CODE_RE = re.compile(r"codigo_fferj=(\d+)")


def merge_key(row: dict) -> str:
    """Chave estável de identidade para o merge.

    O match_id (codigo_fferj) vem fixo do card da FFERJ e não muda quando
    a categoria/órgão/estádio são preenchidos depois pela federação. Usar
    essa chave evita duplicar o mesmo jogo.
    """
    m = FFERJ_CODE_RE.search(row.get("extra", "") or "")
    if m:
        return f"fferj:{m.group(1)}"
    return row_id(row)
