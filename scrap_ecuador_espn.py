...
# -*- coding: utf-8 -*-
"""
Equador - via API pública (não-oficial, mas amplamente usada e estável)
da ESPN.
...
LigaPro do Equador (liga "ecu.1"), entre muitas outras.
...
Uso:
    python scrap_ecuador_espn.py --dias 60 --dias-atras 15
"""
...
def event_to_partido(event: dict, competicao_label: str) -> Partido | None:
    ...
    if not date_iso:
        return None
    data_local, hora_local = utc_iso_to_ecuador(date_iso)
    ...
    return Partido(
        fonte="ESPN",
        competicao=f"Ecuador - {competicao_label}",
        data=data_local,
        hora=hora_local,
        mandante=mandante,
        visitante=visitante,
...
def main() -> None:
    ...
    (OUT_DIR / "debug_ecuador_espn_pages.json").write_text(json.dumps(debug_pages, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "debug_ecuador_espn_raw.json").write_text(json.dumps(rows_new, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    print("")
    print(f"Equador (ESPN) jogos válidos adicionados/atualizados: {len(rows_new)}")
