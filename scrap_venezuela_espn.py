...
# -*- coding: utf-8 -*-
"""
Venezuela - via API pública (não-oficial, mas amplamente usada e estável)
da ESPN.

Descoberta em 17/07/2026: a Venezuela era o único país sul-americano
coberto por outros scrapers deste projeto (Argentina, Bolívia, Brasil,
Chile, Colômbia, Equador, Paraguai, Peru, Uruguai) que não tinha NENHUM
scraper próprio - nem sequer uma base de estádios (ver nota em
estadios_conmebol.js). A federação venezuelana (FVF) não tem um site
oficial fácil de raspar de forma automatizada e estável, então, seguindo
o mesmo padrão já usado com sucesso pra Bolívia/Colômbia/Equador, a fonte
escolhida foi a API JSON que o próprio site da ESPN usa para seus widgets
de placar/calendário:
...
Liga FUTVE (liga "ven.1"), entre muitas outras.
...
Venezuela usa fuso fixo VET (UTC-4, sem horário de verão desde 2016),
igual à Bolívia - reaproveitamos a mesma lógica de conversão.

Uso:
    python scrap_venezuela_espn.py --dias 60 --dias-atras 15
"""
...
LIGAS = [
    ("ven.1", "Liga FUTVE (Primera División)"),
    ("ven.2", "Liga FUTVE 2 (Segunda División)"),
    ("ven.copa", "Copa Venezuela"),
]
# Nota: "ven.1" confirmado com dados atuais (2026) via busca. "ven.2" e
# "ven.copa" existem como slugs (achados em páginas antigas da ESPN, não
# confirmados com dados de 2026) - mantidos no scraper seguindo o mesmo
# princípio já usado em bol.copa: se não tiverem jogos no período
# raspado, simplesmente não trazem nada (sem erro), e voltam a funcionar
# sozinhos se/quando a ESPN atualizar esses dados.
...
def utc_iso_to_venezuela(date_iso: str) -> tuple[str, str]:
    """Venezuela é fixa em UTC-4 (VET, sem horário de verão desde 2016)."""
    try:
...
def event_to_partido(event: dict, competicao_label: str) -> Partido | None:
    ...
    if not date_iso:
        return None
    data_local, hora_local = utc_iso_to_venezuela(date_iso)
    ...
    return Partido(
        fonte="ESPN",
        competicao=f"Venezuela - {competicao_label}",
        data=data_local,
        hora=hora_local,
        mandante=mandante,
        visitante=visitante,
...
def main() -> None:
    ...
    (OUT_DIR / "debug_venezuela_espn_pages.json").write_text(json.dumps(debug_pages, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "debug_venezuela_espn_raw.json").write_text(json.dumps(rows_new, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    print("")
    print(f"Venezuela (ESPN) jogos válidos adicionados/atualizados: {len(rows_new)}")
