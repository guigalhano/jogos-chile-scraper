...
# -*- coding: utf-8 -*-
"""
Bolívia - via API pública (não-oficial, mas amplamente usada e estável)
da ESPN.

A página oficial da seleção boliviana ("La Verde") só existe no
Facebook, que bloqueia qualquer acesso automatizado por robots.txt
(sem exceção, nem com login seria confiável/dentro dos termos de uso).
Por isso, assim como já foi feito para a Colômbia, a fonte escolhida
foi a API JSON que o próprio site da ESPN usa para seus widgets de
placar/calendário:
...
Liga Profesional Boliviana (liga "bol.1"), entre muitas outras.
...
Uso:
    python scrap_bolivia_espn.py --dias 60 --dias-atras 15
"""
...
def utc_iso_to_bolivia(date_iso: str) -> tuple[str, str]:
    """Bolívia é fixa em UTC-4 (sem horário de verão)."""
    try:
...
def event_to_partido(event: dict, competicao_label: str) -> Partido | None:
    ...
    if not date_iso:
        return None
    data_local, hora_local = utc_iso_to_bolivia(date_iso)
    ...
    return Partido(
        fonte="ESPN",
        competicao=f"Bolivia - {competicao_label}",
        data=data_local,
        hora=hora_local,
        mandante=mandante,
        visitante=visitante,
...
def main() -> None:
    ...
    (OUT_DIR / "debug_bolivia_espn_pages.json").write_text(json.dumps(debug_pages, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "debug_bolivia_espn_raw.json").write_text(json.dumps(rows_new, ensure_ascii=False, indent=2), encoding="utf-8")
