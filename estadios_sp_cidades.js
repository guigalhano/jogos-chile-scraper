/*
  Coordenadas de cidades do estado de São Paulo (fallback por cidade).

  A Copa Paulista (e outras competições da FPF cujo PDF só traz a cidade-sede,
  não o nome do estádio) precisa de um fallback por cidade parecido com o
  estadios_mg_cidades.js usado para a FMF. Cobre as cidades que já aparecem
  nos dados atuais da Copa Paulista.
*/

window.CIDADES_SP = {
  "marilia": { lat: -22.2139, lng: -49.9458 },
  "jundiai": { lat: -23.1857, lng: -46.8978 },
  "birigui": { lat: -21.2887, lng: -50.3405 },
  "diadema": { lat: -23.6814, lng: -46.6202 },
  "osasco": { lat: -23.5325, lng: -46.7917 },
  "araras": { lat: -22.3573, lng: -47.3842 },
  "ribeirao preto": { lat: -21.1775, lng: -47.8103 },
  "indaiatuba": { lat: -23.0908, lng: -47.2181 },
  "sao jose dos campos": { lat: -23.2237, lng: -45.9009 },
  "lins": { lat: -21.6797, lng: -49.7422 },
  "santo andre": { lat: -23.6639, lng: -46.5383 },
  "sao paulo": { lat: -23.5505, lng: -46.6333 },
  "bauru": { lat: -22.3246, lng: -49.0871 },
  "presidente prudente": { lat: -22.1256, lng: -51.3889 },
  "piracicaba": { lat: -22.7253, lng: -47.6492 },
  "sao caetano do sul": { lat: -23.6229, lng: -46.5546 },
  "sao bernardo do campo": { lat: -23.6939, lng: -46.5650 },
  "taubate": { lat: -23.0264, lng: -45.5553 },
  "campinas": { lat: -22.9099, lng: -47.0626 },
  "sorocaba": { lat: -23.5015, lng: -47.4526 },
  "santos": { lat: -23.9608, lng: -46.3336 },
  "guarulhos": { lat: -23.4628, lng: -46.5333 },
  "mogi das cruzes": { lat: -23.5228, lng: -46.1883 },
  "franca": { lat: -20.5386, lng: -47.4008 },
  "votuporanga": { lat: -20.4225, lng: -49.9739 },
  "leme": { lat: -22.1875, lng: -47.3903 },
  "barueri": { lat: -23.5106, lng: -46.8761 },
  "braganca paulista": { lat: -22.9530, lng: -46.5417 },
};
