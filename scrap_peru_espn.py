...
# -*- coding: utf-8 -*-
"""
Peru - via API pública (não-oficial, mas amplamente usada e estável) da
ESPN.

Motivo: o scraper existente (scrap_futbolperuano.py, fonte
futbolperuano.com) lê um widget "Powered by Score24" que só expõe a
rodada atual/recente - não o calendário completo da temporada. Isso faz
com que os jogos do Peru em jogos_programados.json fiquem sempre numa
janela curta (poucos dias à frente), diferente de todos os outros
países, que enxergam semanas/meses adiante. Confirmado comparando com
Flashscore, que já lista jogos da Liga 1 até 01-03/08 que não aparecem
na fonte atual.

Este scraper complementa (não substitui) o futbolperuano.com, usando a
mesma API pública da ESPN já usada para Colômbia/Bolívia/Equador:

    https://site.api.espn.com/apis/site/v2/sports/soccer/{liga}/scoreboard?dates=YYYYMMDD

Liga confirmada: "per.1" (Liga 1 - Primera División). Peru é fixo em
UTC-5 (sem horário de verão).

Nota: a ESPN normalmente não cobre a Liga 2/Liga 3/Copa Perú/Femenino
peruanas (apenas a primeira divisão), então essas continuam vindo
apenas do futbolperuano.com. Se no futuro a ESPN passar a cobrir mais
alguma dessas competições, é só adicionar o slug em LIGAS.

Uso:
    python scrap_peru_espn.py --dias 60 --dias-atras 15
"""
...
# Nota: "per.1" e' o slug confirmado (ver espn.com/soccer/league/_/name/per.1
# e espn.com/soccer/scoreboard/_/league/per.1). Outros slugs candidatos
# (ex.: "per.2" para a Liga 2) nao foram confirmados como existentes na
# ESPN e nao devem ser adicionados sem testar primeiro (mesmo padrao
# adotado para "bol.2", que retornava erro e foi removido).
...
def utc_iso_to_peru(date_iso: str) -> tuple[str, str]:
    """Peru é fixo em UTC-5 (sem horário de verão)."""
    try:
...
def event_to_partido(event: dict, competicao_label: str) -> Partido | None:
    ...
    if not date_iso:
        return None
    data_local, hora_local = utc_iso_to_peru(date_iso)
...
def main() -> None:
    ...
    (OUT_DIR / "debug_peru_espn_pages.json").write_text(json.dumps(debug_pages, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "debug_peru_espn_raw.json").write_text(json.dumps(rows_new, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    print("")
    print(f"Peru (ESPN) jogos válidos adicionados/atualizados: {len(rows_new)}")
